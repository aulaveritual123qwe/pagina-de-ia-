...
test('video API validates requests and uses A2E for image-to-video', async () => {
  ...
  try {
    ...
    assert.deepEqual(await (await POST(request({ prompt: 'Animate the scene', mode: 'image', duration: 12, referenceImage: 'data:image/png;base64,dGVzdA==' }))).json(), { taskId: 'a2e:image-task' });
    ...
    globalThis.fetch = async () => Response.json({ output: { task_status: 'SUCCEEDED', video_url: 'https://example.com/video.mp4' } });
    const response = await GET(new Request('https://studio.example/api/generate-video?taskId=test-task'));
