...
export async function POST(request: Request) {
  try {
    ...
    if (typeof body.name !== 'string' || !body.name.trim() || body.name.length > 40 || !Array.isArray(body.references) || body.references.length !== 20 || body.references.some((url: unknown) => typeof url !== 'string' || !url.startsWith(`${new URL(request.url).origin}/api/image/`))) {
